...
export const PostsList: React.FC<PostsListProps> = memo(
  ({ isFeed, currentPosts, isLoadMore, initialPostIndex }) => {
    const navigation = useNavigation();
    const dispatch = useDispatch();
    const [navigatedOutOfScreen, setNavigatedOutOfScreen] = useState<boolean>(
      false
    );
    ...
    const posts: IPost[] = currentPosts
      ? currentPosts
      : isFeed
      ? latestFetchedPosts
      : userPosts;
    ...
    const viewHeight = useMemo(
      () =>
        isFeed
          ? Dimensions.get("window").height - UIConsts.bottomNavbarHeight

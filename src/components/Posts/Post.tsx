      ...
      <TouchableWithoutFeedback onPress={() => onLikeButtonPress()}>
        <View style={[styles.rowContainer, { alignItems: "center" }]}>
          <FontAwesome
            name={"heart"}
            size={22}
            color={isUserLikeChallenge ? Colors.red : Colors.lightGrey}
          />
...
export const Post: React.FC<PostProps> = memo(
  ({ post, isVisible, containerStyle }) => {
    ...
    const onCommentButtonPress = () => {
      if (loggedUser) {
        console.log(
          "user:" + loggedUser?.fullName + " click on comment button."
        );
        // todo: fetch here
      } else {
        navigation.navigate("NotLoggedIn");
    ...
    const onCameraPress = async () => {
      if (loggedUser) {
        console.log(
          "user:" + loggedUser?.fullName + " click on camera button."
        );
